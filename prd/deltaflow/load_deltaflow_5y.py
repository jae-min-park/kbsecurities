...
def showDeltaflow(date = str(datetime.now())[:10], month=9, first_day ='2021-02-01', y5_day='2021-02-22') :
    ...
    for i in range(1, month):
        for j in range(21) :
            start_date = calendar.loc[i,'30Y']-pd.Timedelta(days=3)
    ...
    last_df.loc[0,:] /= cnt
    last_df.loc[1,:] /= cnt30y
    last_df.loc[2,:] /= cnt30y_1
    last_df.loc[3,:] /= cnt30y_2
    last_df.loc[4,:] /= cnt30y_3
    last_df.loc[5,:] /= cnt30y_4
    last_df.loc[6,:] /= cnt3y
    last_df.loc[7,:] /= cnt3y_1
    last_df.loc[8,:] /= cnt3y_2
    last_df.loc[9,:] /= cnt3y_3
    last_df.loc[10,:] /= cnt3y_4
    last_df.loc[11,:] /= cnt10y
    last_df.loc[12,:] /= cnt10y_1
    last_df.loc[13,:] /= cnt10y_2
    last_df.loc[14,:] /= cnt10y_3
    last_df.loc[15,:] /= cnt10y_4
    last_df.loc[16,:] /= cnt5y
    last_df.loc[17,:] /= cnt5y_1
    last_df.loc[18,:] /= cnt5y_2
    last_df.loc[19,:] /= cnt5y_3
    last_df.loc[20,:] /= cnt5y_4
    ...
    for j in range(21) :    
        ...
        if j == 1:
            ...
            if move_date == 0 :
                calendar.loc[month,'30Y']=move_date = start_date
                dates.append('공백')
        ...
        if j == 6:
            ...
            if move_date == 0:
                calendar.loc[month,'3Y']=move_date = calendar.loc[month,'30Y+4']
                dates.append('공백')
                # dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            elif pd.Timestamp(date) <= move_date : 
            # elif pd.Timestamp('2021-08-31') <= move_date : 
                if flag != True:
                    flag = True
                    target_idx = j-1
                    target_date = calendar.loc[month,'30Y+4']
                    dates.append(str(move_date)[5:10])
                else:
                    move_date = calendar.loc[month,'30Y+4']            
                    dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
        ...
        if j == 20:
            ...
            # 5Y+4 이후 입찰이 없는 주라면
            if calendar.loc[month,'N'] != 0 and calendar.loc[month,'N'] <= pd.Timestamp(date):
                if calendar.loc[month,'N+4'] <= pd.Timestamp(date):
                    target_date = move_date = calendar.loc[month,'N+3']
                elif calendar.loc[month,'N+3'] <= pd.Timestamp(date):
                    target_date = move_date = calendar.loc[month,'N+2']
                elif calendar.loc[month,'N+2'] <= pd.Timestamp(date):
                    target_date = move_date = calendar.loc[month,'N+1']
                elif calendar.loc[month,'N+1'] <= pd.Timestamp(date):
                    target_date = move_date = calendar.loc[month,'N']
                elif calendar.loc[month,'N'] <= pd.Timestamp(date):
