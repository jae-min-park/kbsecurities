...
def makeCalendar(first_day ='2021-01-01', y5_day='2020-12-07') :
    ...
    cols = ['10Y','10Y+1','10Y+2','10Y+3','10Y+4',
...
def showDeltaflow(month=7, first_day ='2021-01-01', y5_day='2020-12-07') :
    ...
    for i in range(1, month):
        for j in range(21) :
            start_date = calendar.loc[i,'10Y']-pd.Timedelta(days=3)
    ...
    last_df.loc[2,:] /= cnt10y_1
    last_df.loc[3,:] /= cnt10y_2
    last_df.loc[4,:] /= cnt10y_3
    last_df.loc[5,:] /= cnt10y_4
    last_df.loc[6,:] /= cnt5y
    last_df.loc[7,:] /= cnt5y_1
    last_df.loc[8,:] /= cnt5y_2
    last_df.loc[9,:] /= cnt5y_3
    last_df.loc[10,:] /= cnt5y_4
    last_df.loc[11,:] /= cnt30y
    last_df.loc[12,:] /= cnt30y_1
    last_df.loc[13,:] /= cnt30y_2
    last_df.loc[14,:] /= cnt30y_3
    last_df.loc[15,:] /= cnt30y_4
    last_df.loc[16,:] /= cnt3y
    last_df.loc[17,:] /= cnt3y_1
    last_df.loc[18,:] /= cnt3y_2
    last_df.loc[19,:] /= cnt3y_3
    last_df.loc[20,:] /= cnt3y_4
    ...
    dates=[]
    for j in range(21) :    
        start_date = calendar.loc[month,'10Y']-pd.Timedelta(days=3)
        while start_date in np.array(holi['DATES']) or start_date.day_name() == 'Saturday' or start_date.day_name() == 'Sunday' :
            start_date = start_date - pd.Timedelta(days=1)
        move_date = ''
        if j == 0:
    ...
    for df in dfs :
        ...
        ax.set_title(title)
        ax.set_ylabel('억원')
        ax.set_xlabel(f"\n {str(calendar.loc[month,'10Y'])[5:10]} : 10Y입찰 / {str(calendar.loc[month,'5Y'])[5:10]} : 5Y입찰 / {str(calendar.loc[month,'30Y'])[5:10]} : 30Y입찰 / {str(calendar.loc[month,'3Y'])[5:10]} : 3Y입찰")
        ax.xaxis.grid(True)
        ax.yaxis.grid(True)
