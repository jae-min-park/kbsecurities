...
def showDeltaflow(month=7, first_day ='2021-01-01', y5_day='2020-12-07') :
    ...
    for i in range(1, month):
        for j in range(21) :
            start_date = calendar.loc[i,'5Y']-pd.Timedelta(days=3)
    ...
    last_df.loc[0,:] /= cnt
    last_df.loc[1,:] /= cnt5y
    last_df.loc[2,:] /= cnt5y_1
    last_df.loc[3,:] /= cnt5y_2
    last_df.loc[4,:] /= cnt5y_3
    last_df.loc[5,:] /= cnt5y_4
    last_df.loc[6,:] /= cnt30y
    last_df.loc[7,:] /= cnt30y_1
    last_df.loc[8,:] /= cnt30y_2
    last_df.loc[9,:] /= cnt30y_3
    last_df.loc[10,:] /= cnt30y_4
    last_df.loc[11,:] /= cnt3y
    last_df.loc[12,:] /= cnt3y_1
    last_df.loc[13,:] /= cnt3y_2
    last_df.loc[14,:] /= cnt3y_3
    last_df.loc[15,:] /= cnt3y_4
    last_df.loc[16,:] /= cnt10y
    last_df.loc[17,:] /= cnt10y_1
    last_df.loc[18,:] /= cnt10y_2
    last_df.loc[19,:] /= cnt10y_3
    last_df.loc[20,:] /= cnt10y_4
    ...
    for j in range(21) :
        ...
        if j == 1:
            move_date = calendar.loc[i,'5Y']
            if move_date == 0 :
                calendar.loc[i,'5Y']=move_date = start_date
        ...
        if j == 2:
            move_date = calendar.loc[i,'5Y+1']
            if move_date == 0:
                calendar.loc[i,'5Y+1']=move_date = calendar.loc[i,'5Y']
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            elif pd.Timestamp(str(datetime.now())[:10]) <= move_date : 
                if flag != True:
                    flag = True
                    target_idx = j-1
                    target_date = calendar.loc[i,'5Y']
                move_date = calendar.loc[i,'5Y']                       
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            else :
                dates.append(str(move_date)[5:10])            
        if j == 3:
            move_date = calendar.loc[i,'5Y+2']
            if move_date == 0:
                calendar.loc[i,'5Y+2']=move_date = calendar.loc[i,'5Y+1']
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            elif pd.Timestamp(str(datetime.now())[:10]) <= move_date : 
                if flag != True:
                    flag = True
                    target_idx = j-1
                    target_date = calendar.loc[i,'5Y+1']
                move_date = calendar.loc[i,'5Y+1']                        
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            else :
                dates.append(str(move_date)[5:10])            
        if j == 4:
            move_date = calendar.loc[i,'5Y+3']
            if move_date == 0:
                calendar.loc[i,'5Y+3']=move_date = calendar.loc[i,'5Y+2']
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            elif pd.Timestamp(str(datetime.now())[:10]) <= move_date : 
                if flag != True:
                    flag = True
                    target_idx = j-1
                    target_date = calendar.loc[i,'5Y+2']
                move_date = calendar.loc[i,'5Y+2']                        
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            else :
                dates.append(str(move_date)[5:10])            
        if j == 5:
            ...
            if calendar.loc[i,'N+4'] != 0:
                ...
                calendar.loc[i,'5Y+4']=move_date = calendar.loc[i,'5Y+3']
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            elif pd.Timestamp(str(datetime.now())[:10]) <= move_date : 
                if flag != True:
                    flag = True
                    target_idx = j-1
                    target_date = calendar.loc[i,'5Y+3']
                move_date = calendar.loc[i,'5Y+3']                        
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            else :
                dates.append(str(move_date)[5:10])            
        if j == 6:
            move_date = calendar.loc[i,'30Y']
            if move_date == 0:
                calendar.loc[i,'30Y']=move_date = calendar.loc[i,'5Y+4']
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            elif pd.Timestamp(str(datetime.now())[:10]) <= move_date : 
                if flag != True:
                    flag = True
                    target_idx = j-1
                    target_date = calendar.loc[i,'5Y+4']
                move_date = calendar.loc[i,'5Y+4']            
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            else :
                dates.append(str(move_date)[5:10])            
        if j == 7:
            move_date = calendar.loc[i,'30Y+1']
            if move_date == 0:
                calendar.loc[i,'30Y+1']=move_date = calendar.loc[i,'30Y']
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            elif pd.Timestamp(str(datetime.now())[:10]) <= move_date : 
                if flag != True:
                    flag = True
                    target_idx = j-1
                    target_date = calendar.loc[i,'30Y']
                move_date = calendar.loc[i,'30Y']
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            else :
                dates.append(str(move_date)[5:10])            
        if j == 8:
            move_date = calendar.loc[i,'30Y+2']
            if move_date == 0:
                calendar.loc[i,'30Y+2']=move_date = calendar.loc[i,'30Y+1']
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            elif pd.Timestamp(str(datetime.now())[:10]) <= move_date : 
                if flag != True:
                    flag = True
                    target_idx = j-1
                    target_date = calendar.loc[i,'30Y+1']
                move_date = calendar.loc[i,'30Y+1']            
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            else :
                dates.append(str(move_date)[5:10])            
        if j == 9:
            move_date = calendar.loc[i,'30Y+3']
            if move_date == 0:
                calendar.loc[i,'30Y+3']=move_date = calendar.loc[i,'30Y+2']
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            elif pd.Timestamp(str(datetime.now())[:10]) <= move_date : 
                if flag != True:
                    flag = True
                    target_idx = j-1
                    target_date = calendar.loc[i,'30Y+2']
                move_date = calendar.loc[i,'30Y+2']            
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            else :
                dates.append(str(move_date)[5:10])            
        if j == 10:
            move_date = calendar.loc[i,'30Y+4']
            if move_date == 0:
                calendar.loc[i,'30Y+4']=move_date = calendar.loc[i,'30Y+3']
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            elif pd.Timestamp(str(datetime.now())[:10]) <= move_date : 
                if flag != True:
                    flag = True
                    target_idx = j-1
                    target_date = calendar.loc[i,'30Y+3']
                move_date = calendar.loc[i,'30Y+3']            
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            else :
                dates.append(str(move_date)[5:10])            
        if j == 11:
            move_date = calendar.loc[i,'3Y']
            if move_date == 0:
                calendar.loc[i,'3Y']=move_date = calendar.loc[i,'30Y+4']
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            elif pd.Timestamp(str(datetime.now())[:10]) <= move_date : 
                if flag != True:
                    flag = True
                    target_idx = j-1
                    target_date = calendar.loc[i,'30Y+4']
                move_date = calendar.loc[i,'30Y+4']                          
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            else :
                dates.append(str(move_date)[5:10])            
        if j == 12:
            move_date = calendar.loc[i,'3Y+1']
            if move_date == 0:
                calendar.loc[i,'3Y+1']=move_date = calendar.loc[i,'3Y']
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            elif pd.Timestamp(str(datetime.now())[:10]) <= move_date : 
                if flag != True:
                    flag = True
                    target_idx = j-1
                    target_date = calendar.loc[i,'3Y']
                move_date = calendar.loc[i,'3Y']                          
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            else :
                dates.append(str(move_date)[5:10])            
        if j == 13:
            move_date = calendar.loc[i,'3Y+2']
            if move_date == 0:
                calendar.loc[i,'3Y+2']=move_date = calendar.loc[i,'3Y+1']
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            elif pd.Timestamp(str(datetime.now())[:10]) <= move_date : 
                if flag != True:
                    flag = True
                    target_idx = j-1
                    target_date = calendar.loc[i,'3Y+1']
                move_date = calendar.loc[i,'3Y+1']                          
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            else :
                dates.append(str(move_date)[5:10])            
        if j == 14:
            move_date = calendar.loc[i,'3Y+3']
            if move_date == 0:
                calendar.loc[i,'3Y+3']=move_date = calendar.loc[i,'3Y+2']
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            elif pd.Timestamp(str(datetime.now())[:10]) <= move_date : 
                if flag != True:
                    flag = True
                    target_idx = j-1
                    target_date = calendar.loc[i,'3Y+2']
                move_date = calendar.loc[i,'3Y+2']                          
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            else :
                dates.append(str(move_date)[5:10])            
        if j == 15:
            move_date = calendar.loc[i,'3Y+4']
            if move_date == 0:
                calendar.loc[i,'3Y+4']=move_date = calendar.loc[i,'3Y+3']
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            elif pd.Timestamp(str(datetime.now())[:10]) <= move_date : 
                if flag != True:
                    flag = True
                    target_idx = j-1
                    target_date = calendar.loc[i,'3Y+3']
                move_date = calendar.loc[i,'3Y+3']              
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            else :
                dates.append(str(move_date)[5:10])            
        if j == 16:
            move_date = calendar.loc[i,'10Y']
            if move_date == 0:
                calendar.loc[i,'10Y']=move_date = calendar.loc[i,'3Y+4']
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            elif pd.Timestamp(str(datetime.now())[:10]) <= move_date : 
                if flag != True:
                    flag = True
                    target_idx = j-1
                    target_date = calendar.loc[i,'3Y+4']
                move_date = calendar.loc[i,'3Y+4']  
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            else :
                dates.append(str(move_date)[5:10])            
        if j == 17:
            move_date = calendar.loc[i,'10Y+1']
            if move_date == 0:
                calendar.loc[i,'10Y+1']=move_date = calendar.loc[i,'10Y']
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            elif pd.Timestamp(str(datetime.now())[:10]) <= move_date : 
                if flag != True:
                    flag = True
                    target_idx = j-1
                    target_date = calendar.loc[i,'10Y']
                move_date = calendar.loc[i,'10Y']              
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            else :
                dates.append(str(move_date)[5:10])            
        if j == 18:
            move_date = calendar.loc[i,'10Y+2']
            if move_date == 0:
                calendar.loc[i,'10Y+2']=move_date = calendar.loc[i,'10Y+1']
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            elif pd.Timestamp(str(datetime.now())[:10]) <= move_date : 
                if flag != True:
                    flag = True
                    target_idx = j-1
                    target_date = calendar.loc[i,'10Y+1']
                move_date = calendar.loc[i,'10Y+1']            
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            else :
                dates.append(str(move_date)[5:10])            
        if j == 19:
            move_date = calendar.loc[i,'10Y+3']
            if move_date == 0 :
                calendar.loc[i,'10Y+3']=move_date = calendar.loc[i,'10Y+2']
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            elif pd.Timestamp(str(datetime.now())[:10]) <= move_date : 
                if flag != True:
                    flag = True
                    target_idx = j-1
                    target_date = calendar.loc[i,'10Y+2']
                move_date = calendar.loc[i,'10Y+2']
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            else :
                dates.append(str(move_date)[5:10])            
        if j == 20:
            move_date = calendar.loc[i,'10Y+4']
            if move_date == 0 :
                calendar.loc[i,'10Y+4']=move_date = calendar.loc[i,'10Y+3']
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            elif pd.Timestamp(str(datetime.now())[:10]) <= move_date : 
                if flag != True:
                    flag = True
                    target_idx = j-1
                    target_date = calendar.loc[i,'10Y+3']
                move_date = calendar.loc[i,'10Y+3']
                dates.append(str(move_date+pd.Timedelta(days=1))[5:10])
            else :
                dates.append(str(move_date)[5:10]) 
    ...
    for df in dfs :
        ...
        ax.set_title(title)
        ax.set_ylabel('억원')
        ax.set_xlabel(f"\n {str(calendar.loc[month,'5Y'])[5:10]} : 5Y입찰 / {str(calendar.loc[month,'30Y'])[5:10]} : 30Y입찰 / {str(calendar.loc[month,'3Y'])[5:10]} : 3Y입찰 / {str(calendar.loc[month,'10Y'])[5:10]} : 10Y입찰")
        ax.xaxis.grid(True)
        ax.yaxis.grid(True)
