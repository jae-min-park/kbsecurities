...
def insertExcelData(cursor, df):
    for i in range(len(df.index)) :
        ...
        low =str(df.iloc[i,7])        
        sql = "insert into `lktb30vol` (code, date, time, vwap, price, open, high, low) values ('"  + code + "','" + date + "','" + time + "','" + vwap + "','"+ price +"','"+ opn +"','"+ high +"','"+ low +"'" + ');'
        cursor.execute(sql)
...
# df_sample = setDfData('2017-12-29','2018-12-31')
df_sample = setDfData('2021-04-18','2021-04-27')
