...
def insertExcelData(cursor, df):
    ...
    for i in range(len(df.index)) :
        ...
        sql = "insert into `usdkrw1vol` (code, date, time, price) values ('"  + code + "','" + date + "','" + time + "','" + price +"'" + ');'
        cursor.execute(sql)
        

def setDfData(date_start, date_end) :
    sql = "SELECT * FROM `usdkrwtick` where date >= '"+ str(date_start)[:10] + "' and date <= '" + str(date_end)[:10] + "';"
    cursor.execute(sql)
    result = cursor.fetchall()
    return pd.DataFrame(result)

df_sample = setDfData('2017-12-20','2018-03-31')
print(df_sample)
