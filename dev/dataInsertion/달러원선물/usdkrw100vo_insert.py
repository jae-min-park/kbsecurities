...
def insertExcelData(cursor, df):
    for i in range(len(df.index)) :
        ...
        low =str(df.iloc[i,7])        
        sql = "insert into `usdkrw100vol` (code, date, time, vwap, price, open, high, low) values ('"  + code + "','" + date + "','" + time + "','" + vwap + "','"+ price +"','"+ opn +"','"+ high +"','"+ low +"'" + ');'
        cursor.execute(sql)
        

def setDfData(date_start, date_end) :
    sql = "SELECT * FROM `usdkrw1vol` where date >= '"+ str(date_start)[:10] + "' and date <= '" + str(date_end)[:10] + "';"
    cursor.execute(sql)
    result = cursor.fetchall()
    return pd.DataFrame(result)

df_sample = setDfData('2017-12-18','2018-03-30')
