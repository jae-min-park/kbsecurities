...
def afternoonRise(date, db_table, af_config, method="simple", plot="Y"):
    """
    오후 고가 갱신하는 경우 추격매수하는 모멘텀 트레이딩
    
    발동조건
        1. 시간조건, 오후 2pm or ?
        2. 현재가 > 이전 고가
        3. LONG by ema, 약세분위기 감지용
    ...
    #장중고가 상향돌파 margin(틱), +2 --> 장중가보다 2틱 위를 돌파로 가정
    thru = af_config['thru']
    ...
    open_chg_1d = round(dfmkt.iloc[0]['close'] - yday_close, 2)
    ...
    for dti_pre, dti_now in zip(dti, dti[1:]):
        ...
        # losscut 발동, losscut 실행전
            ...
            bangla['pl'] = round(product_multiplier*(bangla['losscut_price'] - bangla['trade_price']), 1)
            break
        ...
        # profit-taking 발동, pt실행전
            ...
            bangla['pl'] = round(product_multiplier*(bangla['pt_price'] - bangla['trade_price']), 1)
            break

        # 매매실행이후 PL 기록, 종가PL도 아래에서 동일하게 처리
        elif bangla['is_triggered'] == True and bangla['is_traded'] == True:
            bangla['pl'] = round(product_multiplier*(close_price - bangla['trade_price']), 1)
            bangla['max_gain'] = max(bangla['max_gain'], bangla['pl'])
            bangla['draw_down'] = bangla['pl'] / bangla['max_gain']
            # print(now, bangla['max_gain'], bangla['pl'], bangla['draw_down'])
            
            # 손절 로직1 : 매매발동전 관찰기간중 고가-저가의 ???% 폭만큼 하락하면 손절 트리거
            # 손절 로직2 : 30틱
            # 1 or 2 만족시 손절
            ...
            bool_draw_down = af_config['pt_draw_down'] < (1 - bangla['pl'] / bangla['max_gain'])
        ...
        # 매매시간대 진입, 본격 조건 탐색            
            ...
            if vwap > (intra_hi + thru/product_multiplier) and tested_status == "above":
    ...
    if bangla['is_traded'] == True and plot == "Y":
        ...
        # 장중 고가 H-line
        plt.axhline(y=intra_hi, color='r', linestyle='dashed')
...
af_config = {'trading_begins_after': datetime.time(9,15,0),
             'trading_begins_before': datetime.time(9,50,0),
             'ema_fast_coeff': 0.20,
             'ema_slow_coeff': 0.05,
             'thru': 0.5,
             'ema_margin': 0.5,
...
for i, day in enumerate(ld):
    ...
    result = afternoonRise(day, af_config=af_config, db_table='lktbf100vol', plot="Y")
