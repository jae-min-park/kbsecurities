...
def afternoonFall(date, db_table, af_config, method="simple", plot="Y"):
    ...
    for dti_pre, dti_now in zip(dti, dti[1:]):
        ...
        # 현재가
        last_price = dfmkt.loc[dti_now,'close']
        
        # ema 기록
        dfmkt.at[dti_now, 'ema_fast'] = fast_coeff * last_price + (1-fast_coeff) * dfmkt.at[dti_pre, 'ema_fast']
        dfmkt.at[dti_now, 'ema_slow'] = slow_coeff * last_price + (1-slow_coeff) * dfmkt.at[dti_pre, 'ema_slow']
        ...
        if uno.is_triggered and not uno.is_traded:
            uno.trade_price = last_price
            uno.trade_time = now
            uno.trade_local_index = dti_now
            uno.is_traded = True
            # max_gain 초기화
            uno.max_gain = 0
            uno.max_gain_price = last_price
        ...
        if now < trading_begins_after:
            intra_lo = min(intra_lo, last_price)
            intra_hi = max(intra_hi, last_price)
        ...
        # 매매시간대 진입, 본격 조건 탐색            
            ...
            if last_price < (intra_lo - thru/uno.product_multiplier) and tested_status == "below":
    ...
    if time_break < trading_begins_before and (uno.is_losscut_traded or uno.is_pt_traded):
        ...
        for dti_pre, dti_now in zip(dti[dti_break:], dti[dti_break+1:]):
            ...
            # ema 기록
            dfmkt.at[dti_now, 'ema_fast'] = fast_coeff * last_price + \
                (1-fast_coeff) * dfmkt.at[dti_pre, 'ema_fast']
            
            dfmkt.at[dti_now, 'ema_slow'] = slow_coeff * last_price + \
            ...
            # 매매 조건 탐색
            if last_price < dos_entry and tested_status == "below":
    ...
    if uno.is_traded and plot == "Y":
        ...
        plt.plot(dti, dfmkt['close'], linewidth=0.5)
        plt.plot(dti, dfmkt['ema_fast'])
        plt.plot(dti, dfmkt['ema_slow'])
        
        from scipy.signal import argrelextrema
        peak_around = 6*10
        dfmkt['lo'] = dfmkt.iloc[argrelextrema(dfmkt.close.values, 
                                                np.less_equal,
                                                # np.less,
                                                order= peak_around, mode='wrap')[0]]['close']
        dfmkt['hi'] = dfmkt.iloc[argrelextrema(dfmkt.close.values, 
                                                np.greater_equal,
                                                # np.greater,
                                                order= peak_around, mode='wrap')[0]]['close']
        plt.scatter(dti, dfmkt['lo'], c='steelblue')
        plt.scatter(dti, dfmkt['hi'], c='orangered')
...
for i, day in enumerate(ld):
    ...
    uno, dos, dfmkt = afternoonFall(day, af_config=af_config, db_table='lktbf_10sec', plot="Y")
    ...
    siga_chg = 100 * (dfmkt.iloc[0]['open'] - util.getYdayOHLC(day, table='lktbf_day')['close'])
    dfpl.at[i, 'siga_chg'] = siga_chg
