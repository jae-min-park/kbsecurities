...
df = util.setDfData(str(date), str(date2), 'lktbf100vol')
...
for dt in li :
    ...
    for pre_i, post_i in zip(dti, dti[80:]) :
        ...
        margin = sd/8
        loss_cut = sd/2
        price = df_today.loc[post_i, 'close']
        ...
        # 포지션이 없을때 진입을 포지션 롱으로 하나 결정(롱에서 loss cut 했으면 진입 롱 안함)
            ...
            x.append(post_i)
            y.append(df_today.loc[post_i,'close'])
    ...
    plt.plot(df_today.index, df_today['close'])
